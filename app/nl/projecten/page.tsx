...
import { PROJECTS_NL } from "../../data/projects";

/*
  Projecten — YellowSun Power (Dutch mirror of app/realisations/page.tsx)
  Same structure as the French/English versions: an intro, a type
  filter, and a static grid of full case-study cards built from
  PROJECTS_NL.
*/
...
function ProjectCard({ project }: { project: (typeof PROJECTS_NL)[number] }) {
  return (
...
export default function ProjectenPageNL() {
  const [filter, setFilter] = useState("Alle");

  const types = useMemo(
    () => ["Alle", ...Array.from(new Set(PROJECTS_NL.map((p) => p.type)))],
    []
  );

  const filtered = useMemo(
    () => (filter === "Alle" ? PROJECTS_NL : PROJECTS_NL.filter((p) => p.type === filter)),
    [filter]
  );
        ...
        <section className="relative w-full bg-[#14120F] pb-16 pt-32 sm:pb-20 sm:pt-40 lg:pb-24 lg:pt-48">
          <div className="mx-auto max-w-[1600px] px-6 sm:px-10 lg:px-14 xl:px-20">
            ...
            <h1 className="mt-6 max-w-2xl text-[clamp(2rem,3.4vw+1rem,3.6rem)] font-medium leading-[1.1] tracking-tight text-[#F5EFE3]">
              Elk dak is een uniek geval.
            </h1>

            <p className="mt-6 max-w-xl text-base leading-relaxed text-[#F5EFE3]/70 sm:text-lg">
              Dimensionering, oriëntatie en architecturale integratie
              worden per project bekeken, van villa's in Tanger tot
              industriële sites in Casablanca.
            </p>
          </div>
        </section>
        ...
        <section className="w-full bg-[#14120F] py-24 sm:py-32">
          <div className="mx-auto flex max-w-[1600px] flex-col items-center gap-8 px-6 text-center sm:px-10">
            <h2 className="max-w-2xl text-[clamp(1.7rem,2.8vw+1rem,2.8rem)] font-semibold leading-[1.2] tracking-tight text-[#F5EFE3]">
              Uw dak verdient dezelfde aandacht.
            </h2>
            <ClosingCTA />
          </div>
        </section>
